...
class Board {
    ...
    Board(Board board0) {
        ...
        _numJumpTracker = new ArrayList<>(board0._numJumpTracker);
        _numJumpTracker.add(_numJumps);
        _numMoves = board0.numMoves();
        _numPieces = new int[]{_totalOpen, 0,
                board0.redPieces(), board0.bluePieces()};
    ...
    void clear() {
        ...
        _undoPieces.clear();
        _allMoves.clear();
        _numJumpTracker.clear();
        _winner = null;
        _numMoves = 0;
        _numJumps = 0;
        _numJumpTracker.add(0);
    }
    ...
    boolean legalMove(Move move) {
        if (move == null) {
            ...
            return !canMove(whoseMove());
                ...
                || get(move.col0(), move.row0()) != whoseMove()) {
            return false;
        } else {
            return move.col0() >= 'a' && move.col0() <= 'g'
                    && move.col1() >= 'a' && move.col1() <= 'g';
        }
    }

    boolean legalMove1(Move move) {
        if (move == null) {
            return false;
        } else if (move.isPass()) {
            return !canMove(whoseMove());
    ...
    boolean legalMove1(char c0, char r0, char c1, char r1) {
        return legalMove1(Move.move(c0, r0, c1, r1));
    }
     ...
     */
    int numJumps() {
        return _numJumpTracker.get(_numJumpTracker.size() - 1);
    }
    ...
    void makeMove(Move move) {
        ...
        if (_numJumps == JUMP_LIMIT) {
            if (numPieces(whoseMove()) == numPieces(opponent)) {
                _winner = EMPTY;
            } else if (numPieces(whoseMove()) > numPieces(opponent)) {
                _winner = _whoseMove;
            } else if (numPieces(whoseMove()) < numPieces(opponent)) {
                _winner = opponent;
            }
        } else if (!canMove(whoseMove()) && !canMove(opponent)) {
            if (numPieces(whoseMove()) == numPieces(opponent)) {
                _winner = EMPTY;
            } else if (numPieces(whoseMove()) > numPieces(opponent)) {
                _winner = _whoseMove;
    ...
    void undo() {
        ...
        _numJumpTracker.remove(_numJumpTracker.size() - 1);
        _undoSquares.pop();
        _undoPieces.pop();
        _numMoves--;
        _whoseMove = _whoseMove.opposite();
    ...
    void setBlock(char c, char r) {
        ...
        if (legalBlock(c, (char) ('4' - r + '4'))) {
            set(c, (char) ('4' - r + '4'), BLOCKED);
            _totalOpen--;
        }
        if (legalBlock((char) ('d' - c + 'd'), r)) {
            set((char) ('d' - c + 'd'), r, BLOCKED);
            _totalOpen--;
        }
        if (legalBlock((char) ('d' - c + 'd'), (char) ('4' - r + '4'))) {
            set((char) ('d' - c + 'd'), (char) ('4' - r + '4'), BLOCKED);
            _totalOpen--;
        }
    ...
    /**
     * Number of jumps tracker.
     */
    private ArrayList<Integer> _numJumpTracker;
