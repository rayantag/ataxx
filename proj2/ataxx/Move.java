...
class Move {
    ...
    private Move(int col0, int row0, int col1, int row1) {
        ...
        if (Math.abs(col1 - col0) > 2 || Math.abs(row1 - row0) > 2) {
            _isJump = false;
            _isExtend = false;
        } else if (Math.abs(col1 - col0) == 2 || Math.abs(row1 - row0) == 2) {
            _isExtend = false;
            _isJump = true;
        } else if (Math.abs(col1 - col0) == 1 || Math.abs(row1 - row0) == 1) {
            _isExtend = true;
            _isJump = false;
        }
    }
    ...
    public String toString() {
        if (isPass()) {
            ...
            String cs0 = String.valueOf(col0());
            String rs0 = String.valueOf(row0());
            String cs1 = String.valueOf(col1());
            String rs1 = String.valueOf(row1());
